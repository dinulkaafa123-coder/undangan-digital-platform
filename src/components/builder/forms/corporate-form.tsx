...
import { FormSection, FieldRow, Field, TextInput, TextArea, GalleryUpload, ImageUpload, ListEditor, ArrayToggleRow, MapsFields } from "@/components/builder/fields";

export function CorporateForm({ data, onChange }: { data: CorporateEventData; onChange: (data: CorporateEventData) => void }) {
  const { schedule } = data;
  const speaker = data.speaker ?? { name: "", title: "", photoUrl: "" };

  return (
    <div className="space-y-5">
      <FormSection title="1. Data Perusahaan & Acara">
        <FieldRow>
          <Field label="Nama Perusahaan">
            <TextInput value={data.companyName} onChange={(e) => onChange({ ...data, companyName: e.target.value })} />
          </Field>
          <Field label="Nama Acara">
            <TextInput value={data.title} onChange={(e) => onChange({ ...data, title: e.target.value })} placeholder="mis. Annual Gathering 2026" />
          </Field>
        </FieldRow>
      ...
      <FormSection title="2. Pembicara (Speaker)">
        <Field label="Foto">
      ...
      <FormSection title="4. Susunan Acara (Agenda)">
        <ListEditor
          ...
          renderItem={(item, update) => (
            <FieldRow>
              <Field label="Waktu">
                <TextInput value={item.time} onChange={(e) => update({ time: e.target.value })} placeholder="mis. 09.00 - 09.30" />
              </Field>
      ...
      <FormSection title="6. Registrasi / Kontak">
        <FieldRow>
